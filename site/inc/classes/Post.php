...
class Post {
    ...
    public function __construct($post_id) {
        ...
        $post = $this -> sql_connection -> prepare("SELECT posts.id, posts.thumbnail, posts.title, posts.content, posts.user_id FROM posts WHERE id = :id LIMIT 1");
        $post -> bindParam(':id', $post_id, PDO::PARAM_INT);
        $post -> execute();

        if ($post -> rowCount() == 1) { // if user row exists
    ...
    public function deletePost(int $post_id) { // delete a post if user id is author

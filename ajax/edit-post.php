...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($_POST['title']) && !empty($_POST['content'])) {
        if (!empty($_FILES)) { // if the files super global is empty, don't process it. let the default value of null take over
            ...
            if (in_array($image_extension, $valid_extension)) { // extension validation
                if ($_FILES['thumbnail']['size'] < 1000000) {
                    ...
                    if (move_uploaded_file($temporary_thumbnail, $upload_path)) { // when upload is complete, do the rest
                        ...
                        $addPost = $sql_connection -> prepare("UPDATE posts SET thumbnail = :thumbnail, title = :title, content = :content, edited = :edited WHERE id = :id");
